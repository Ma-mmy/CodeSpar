...
我们使用Spring AI中的modular RAG的方式，来实现检索增强部分。即使用RetrievalAugmentationAdvisor来实现。



因为我们不涉及到更多的优化内容，所以，只需要定制documentRetriever和queryAugmenter就行了。



VectorStoreDocumentRetriever定义，使用vectorStore构建一个retriever即可：
...
提示词中主要是针对图片做了些特殊要求，以及要求输出markdown格式方便渲染。同时需要注意占位符需要和文档中默认的保持一致：

![](../../access/113r87a2a7c04b6ff390f011d0d187f1c029.png)
